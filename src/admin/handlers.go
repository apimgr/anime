...
func (h *Handler) RegisterRoutes(r *mux.Router) {
	...
	r.HandleFunc("/admin", h.handleAdminLogin).Methods("GET")
	r.HandleFunc("/admin/login", h.handleAdminLoginPost).Methods("POST")
	r.HandleFunc("/admin/logout", h.handleAdminLogout).Methods("GET", "POST")
	r.HandleFunc("/admin/dashboard", h.requireSession(h.handleAdminDashboard)).Methods("GET")
	r.HandleFunc("/admin/settings", h.requireSession(h.handleAdminSettings)).Methods("GET")
	r.HandleFunc("/admin/settings", h.requireSession(h.handleAdminSettingsPost)).Methods("POST")

	// Admin API (bearer token auth)
	api := r.PathPrefix("/api/v1/admin").Subrouter()
	api.HandleFunc("/status", h.requireToken(h.handleAPIStatus)).Methods("GET")
	api.HandleFunc("/config", h.requireToken(h.handleAPIGetConfig)).Methods("GET")
	api.HandleFunc("/config", h.requireToken(h.handleAPIUpdateConfig)).Methods("PUT")
	api.HandleFunc("/reload", h.requireToken(h.handleAPIReload)).Methods("POST")
}
...
<head>
    ...
    <title>Admin Login - Anime API</title>
    <style>
        :root {
            --bg-color: #282a36;
            --fg-color: #f8f8f2;
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Admin Dashboard - Anime API</title>
    <style>
        :root {
            --bg-color: #282a36;
            --fg-color: #f8f8f2;
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Settings - Anime API</title>
    <style>
        :root {
            --bg-color: #282a36;
            --fg-color: #f8f8f2;
